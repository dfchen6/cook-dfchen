...
export default function RecipeSteps({ steps }: { steps: RecipeStep[] }) {
  ...
  const [mode, setMode] = useState<'list' | 'slide'>('list');
  ...
  return (
    <div>
      {/* View switcher — list shows every step at once, slide focuses on one. */}
      <div className="mb-4 inline-flex rounded-full border border-stone-200 p-0.5 dark:border-stone-700">
        <button
          type="button"
      ...
      {/* Progress: counter + tappable dots (slide mode only) */}
      {mode === 'slide' && (
        <>
